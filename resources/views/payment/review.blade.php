  ...
  <div class="row g-4">
    <div class="col-lg-5">
      <div class="bg-success text-white rounded p-4 h-100 shadow animate__animated animate__fadeInLeft">
        <h4 class="fw-bold">BilBayaran</h4>
          ...
          <ul class="ps-3 list-unstyled">
            @foreach (
              ...
              <li class="mb-2">
                ...
                <span class="badge {{ $badgeClass }} ms-2">{{ $icon }} {{ strtoupper($student->class_name) }}</span>
              </li>
            @endforeach
          </ul>
        </div>
      ...
      <div class="card shadow-sm border-0 animate__animated animate__fadeInRight">
        <div class="card-body">
          ...
          <form action="{{ route('confirm.payment', ['familyId' => $familyId]) }}" method="POST" id="paymentForm">
            ...
            <div class="d-grid">
              <button type="submit" class="btn btn-success btn-lg">Sahkan & Bayar</button>
            </div>
          </form>
        </div>
      </div>
